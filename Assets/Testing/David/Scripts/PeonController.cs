...
public class PeonController : MonoBehaviour
{
    ...
    void ClosestPawn()
    {
        ...
        if (CompareTag("Player"))
        {
            ...
            for (int i = 0; i < combatManager.GetComponent<CombateManager>().m_enemies.Length; i++)
            {
                distance = (transform.position - combatManager.GetComponent<CombateManager>().m_enemies[i].transform.position).magnitude;
                ...
                if (distance < closestDistance)
                {
                    ...
                    m_positionToGo = combatManager.GetComponent<CombateManager>().m_enemies[i].transform;
                }
            }
        ...
        else if (CompareTag("Enemy"))
        {
            ...
            for (int i = 0; i < combatManager.GetComponent<CombateManager>().m_players.Length; i++)
            {
                distance = (transform.position - combatManager.GetComponent<CombateManager>().m_players[i].transform.position).magnitude;
                ...
                if (distance < closestDistance)
                {
                    ...
                    closestDistance = distance;
                    m_positionToGo = combatManager.GetComponent<CombateManager>().m_players[i].transform;
                }
            }
    ...
    void Attack()
    {
        ...
        if (m_isAlive && m_pawnToAttack.m_isAlive)
        {
            ...
            if (m_pawnToAttack.current_hp < 1)
            {
                ...
                m_pawnToAttack.gameObject.GetComponent<PeonController>().m_isAlive = false;
    ...
    public bool EnemyIsClose()
    {
        for (int i = 0; i < m_directions.Length; i++)
        {
            ...
            if (!GridManager.Instance.IsTileEmpty(positionToCheck))
            {
                if (CompareTag("Player"))
                {
                    for (int j = 0; j < combatManager.GetComponent<CombateManager>().m_enemies.Length; j++)
                    {
                        if (combatManager.GetComponent<CombateManager>().m_enemies[j].GetComponent<PeonController>().m_isAlive)
                        {
                            Vector2 enemyPosition = combatManager.GetComponent<CombateManager>().m_enemies[j].transform.position;

                            if (positionToCheck == GridManager.Instance.ScreenToTilePosition(Camera.main.WorldToScreenPoint(enemyPosition)))
                            {
                                if (combatManager.GetComponent<CombateManager>().m_enemies[j].GetComponent<PeonController>().m_isAlive)
                                {
                                    m_pawnToAttack = combatManager.GetComponent<CombateManager>().m_enemies[j].GetComponent<PeonController>();
                ...
                {
                    for (int j = 0; j < combatManager.GetComponent<CombateManager>().m_players.Length; j++)
                    {

                        Vector2 playerPosition = combatManager.GetComponent<CombateManager>().m_players[j].transform.position;

                        if (positionToCheck == GridManager.Instance.ScreenToTilePosition(Camera.main.WorldToScreenPoint(playerPosition)))
                        {
                            if (combatManager.GetComponent<CombateManager>().m_players[j].GetComponent<PeonController>().m_isAlive)
                            {
                                m_pawnToAttack = combatManager.GetComponent<CombateManager>().m_players[j].GetComponent<PeonController>();
